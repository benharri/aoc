...
{
    private readonly List<(int, int)> _intersections = [];
    private readonly List<Dictionary<(int, int), int>> _wires = [];

    public override void ProcessInput()
    {
        _wires.AddRange(Input.Select(ParseWire));
        _intersections.AddRange(_wires[0].Keys.Intersect(_wires[1].Keys));
    }
    ...
    private static Dictionary<(int, int), int> ParseWire(string line)
    {
        ...
        foreach (var step in line.Split(','))
        {
            ...
            switch (step[0])
            {
                case 'U': for (; i < d; i++) r.TryAdd((x, ++y), c++); break;
                case 'D': for (; i < d; i++) r.TryAdd((x, --y), c++); break;
                case 'R': for (; i < d; i++) r.TryAdd((++x, y), c++); break;
                case 'L': for (; i < d; i++) r.TryAdd((--x, y), c++); break;
            }
        }
