...
public sealed class Day09SmokeBasin() : Day(2021, 9, "Smoke Basin")
{
    ...
    private readonly List<Point2d<int>> _lowPoints = [];
    ...
    public override object Part2()
    {
        ...
        {
            var s = 0;
            var seen = new HashSet<Point2d<int>>();

            MoreEnumerable.TraverseBreadthFirst(point, Traverse).Consume();
            sizes.Add(s);
            continue;

            IEnumerable<Point2d<int>> Traverse(Point2d<int> p)
            {
                var (i, j) = p;
